...
fn r20_lsp_parser(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("r20_lsp_parser");
...
fn r20_lsp_editor_features(c: &mut Criterion) {
    let mut group = c.benchmark_group("r20_lsp_editor_features");
...
fn r20_lsp_diagnostics_and_full_sync(c: &mut Criterion) {
    let mut group = c.benchmark_group("r20_lsp_diagnostics_full_sync");
    let large_document = large_document_with_canonical_call(250);
